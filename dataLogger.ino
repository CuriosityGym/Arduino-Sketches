...
void loop() 
    {
     // in the following function just pass the two parameters. Unit of both parameter is Minute.
     //first parameter is interval at which you want to log the data to SD card.
     //second parameter is duration which is the total time for which you want to log data.
     
     timeInterval(1,15);    // here we are logging data at interval of 1 minute for 15 mintutes, i.e, 15 samples.
...
int timeInterval(int interval,int duration)
   {
      ...
      time = 0;   
      if ((duration >= time) && (time % interval == 0))
          {
...
void LogToSDcard()
  ...
  // to read the content of particular file just comment symbols from the following code (/*  */  <----remove this)
   
   /*
        // re-open the file for reading:
     dataFile = SD.open("LOGGER02.CSV");      // enter the name of file you want to read from.
     if (dataFile)
       {
         Serial.println("LOGGER02.CSV");
