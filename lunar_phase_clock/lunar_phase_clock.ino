...
void get_moonPhase()
    { 
      sprintf(buff, "/apps/thinghttp/send_request?api_key=6018SSES4DFVVRQA");
          ...
          if(code == HTTP_STATUS_OK)     //check for response for HTTP request  
            {
              ...
              moonPhase = response;
              String moon_phase = moonPhase.substring(0, 5);
              u8g.firstPage();
              do { u8g.setFont(u8g_font_timB10);
                   u8g.drawStr(1,10,"Moon Phase:");
                   u8g.setFont(u8g_font_timB14); 
                   u8g.setPrintPos(5, 30);
                   u8g.print(moon_phase);
                 } while( u8g.nextPage() );
            } 
          else 
            {
